...
func (c CertificateStruct) createPrivateKey() (*rsa.PrivateKey, *cerr.CustomError) {
	...
	if pk, err = rsa.GenerateKey(rand.Reader, CertPKsize); err != nil {
		return nil,
			&cerr.CustomError{Title: "Unable to generate the RSA private key", Message: err.Error(), Fatality: cerr.Fatal}
	}

	// rootCA keys are not stored at the same place as other SSL keys
	if c.IsCA {
		if err = os.MkdirAll(env.RootCAdir, os.ModePerm); err != nil {
			return nil, &cerr.CustomError{Message: err.Error(), Fatality: cerr.Fatal}
		}
	...
	if err = pem.Encode(pkFile, pkBlock); err != nil {
		return nil, &cerr.CustomError{Title: "Error encoding RSA private key", Message: err.Error(), Fatality: cerr.Fatal}
	}
	return pk, nil
}
...
func (c CertificateStruct) getPrivateKey(env environment.EnvironmentStruct) (*rsa.PrivateKey, *cerr.CustomError) {
	...
	if pKeyFile, err = os.ReadFile(filepath.Join(keyDir, c.CertificateName+".key")); err != nil {
		return nil,
			&cerr.CustomError{Title: "Error reading the private key: ", Message: err.Error(), Fatality: cerr.Fatal}
	}

	// Decode keyfile
	if key, _ := pem.Decode(pKeyFile); key == nil {
		return nil,
			&cerr.CustomError{Title: "Unable to PEM-decode the private key", Message: err.Error(), Fatality: cerr.Fatal}
	} else {
		// Parse keyfile
		if pkey, err = x509.ParsePKCS1PrivateKey(key.Bytes); err != nil {
			return nil,
				&cerr.CustomError{Title: "Error parsing the private key", Message: err.Error(), Fatality: cerr.Fatal}
		}
	}
	return pkey, nil
}
...
func (c CertificateStruct) generateCSR(env environment.EnvironmentStruct, privateK *rsa.PrivateKey) *cerr.CustomError {
	...
	if err != nil {
		return &cerr.CustomError{Title: "Error creating CSR", Message: err.Error(), Fatality: cerr.Fatal}
	}

	if csrFile, err = os.Create(filepath.Join(env.ServerCertsDir, "csr", c.CertificateName+".csr")); err != nil {
		return &cerr.CustomError{Title: "Error creating CSR file", Message: err.Error(), Fatality: cerr.Fatal}
	}
	defer csrFile.Close()

	if err = pem.Encode(csrFile, &pem.Block{Type: "CERTIFICATE REQUEST", Bytes: certRequest}); err != nil {
		return &cerr.CustomError{Title: "Error encoding CSR", Message: err.Error(), Fatality: cerr.Fatal}
	}
