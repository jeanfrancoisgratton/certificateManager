...
func Verify(certFilePaths []string) *cerr.CustomError {
...
func verifyCert(certFilePath string) *cerr.CustomError {
	...
	if cert == nil {
		return &cerr.CustomError{Message: "Failed to decode certificate PEM block", Fatality: cerr.Warning}
	}
	...
	if CaVerifyComments {
		...
		if e, ce = environment.LoadEnvironmentFile(); ce != nil {
			return ce
		}
		cfgfileName := filepath.Base(certFilePath)
		baseName := cfgfileName[:len(cfgfileName)-len(filepath.Ext(cfgfileName))] + ".json"
		if c, ce = LoadCertificateConfFile(filepath.Join(e.CertificatesConfigDir, baseName)); ce != nil {
			return ce
		}
		if len(c.Comments) > 0 {
